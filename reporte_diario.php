...
// Query para el saldo de Caja
$queryCaja = Controller::$connection->query(
    "
    select c.id as id, c.saldo as saldo
    from caja as c
    where c.fecha = '$cod'
    ORDER by c.id DESC
    LIMIT 1    
...
if($queryCaja->rowCount()){
    $dataCaja = $queryCaja->fetchAll(PDO::FETCH_ASSOC);
    $saldoCaja = $dataCaja[0]["saldo"];
} else {
    $die("Revisar la caja");
}
...
<body>
    ...
    <div class="container">
        <div class="row">
            ...
            <h2 align="left" class="titulos" style="line-height: 6px">  Saldo en Caja : Q. $saldoCaja</h2>  
...
$pdf->Output('ReporteDelDia.pdf', 'I');
